...
"""
    ...
    def _get_document_analysis_prompt(self, document_map: str) -> str:
        # Note: The method signature in PromptManager needs to be updated.
...
3.  **Определи назначение:** Для каждого найденного `run`-а определи его юридическое назначение, ориентируясь на симметричное расположение относительно реквизитов 'Исполнителя'.
4.  **Классифицируй поля:** Различай два типа полей:
    - **Реквизиты организации** — все поля, связанные с официальными данными компании (адрес, налоговые данные, телефоны, банковские реквизиты и т.д.)
    - **Другие поля** — наименование организации, имя руководителя, должность и прочие персональные/корпоративные данные
5.  **Определи зоны реквизитов:** Если видишь таблицу или структурированную область с реквизитами у 'Исполнителя', то все соответствующие пропуски у 'Заказчика' в этой зоне помечай как единую область "реквизиты". Не создавай отдельные поля для каждой строки реквизитов.
6.  **Сформируй "план правок":** Верни **ТОЛЬКО** валидный JSON-массив. Каждый объект в массиве должен представлять один `run`, который нужно изменить, и содержать два ключа:
    - `run_id`: Точный ID фрагмента, который нужно заменить (например, "run-17").
    - `field_name`: Для реквизитов организации используй "реквизиты", для остальных полей — конкретное название (например, "Полное наименование", "Имя руководителя").
...
**ВАЖНЫЕ ПРИНЦИПЫ:**
- Поля, которые должны быть заполнены реквизитами организации, помечай как "реквизиты". Не расписывай конкретные реквизиты — просто указывай, что это место предназначено для реквизитов.
- Если видишь таблицу с реквизитами, все пропуски в соответствующей зоне у 'Заказчика' помечай как "реквизиты" без нумерации.
- Сохраняй структуру и формат документа — не меняй размеры таблиц, просто помечай области для заполнения.
- Ориентируйся на симметрию с заполненной стороной, но не копируй точное количество строк — определяй зоны.
