...
**ЗАДАЧА:** Определи незаполненные поля (с подчеркиваниями или выделениями), которые нужно заполнить. Анализируй документ целиком, понимая его логику и структуру.
...
**1. ЦЕЛОСТНЫЙ АНАЛИЗ ДОКУМЕНТА:**
- Изучи весь документ, чтобы понять его структуру и логику
- Определи стороны договора и их роли
- Найди таблицы реквизитов и проанализируй их как единое целое
- Выяви паттерны заполнения между сторонами

**2. ПРИНЦИП СИММЕТРИИ ДЛЯ ТАБЛИЦ РЕКВИЗИТОВ:**
- **Обязательная симметрия ячеек**: Если в ячейке у одной стороны есть данные, то в аналогичной ячейке у другой стороны ОБЯЗАТЕЛЬНО должен быть маркер "реквизиты"
- **Обязательная симметрия строк**: Если в строке у одной стороны есть данные, то в аналогичной строке у другой стороны ОБЯЗАТЕЛЬНО должен быть маркер "реквизиты"
- **Логика заполнения**: Анализируй, какие реквизиты уже заполнены у одной стороны, и применяй симметрию для незаполненной стороны
- **Контекстная симметрия**: Если у поставщика заполнены 5 строк реквизитов, то у покупателя должны быть заполнены реквизиты в аналогичных 5 строках

**3. ПРАВИЛА ОБРАБОТКИ ТАБЛИЦ:**
- Каждая строка таблицы реквизитов = одно поле "реквизиты"
- Игнорируй форматирование (пробелы, разрывы строк)
- Анализируй контекст: если строка в разделе реквизитов, она предназначена для реквизитов
- Обрабатывай ячейки целиком, убирая лишнее форматирование

**4. ТЕХНИЧЕСКАЯ ОБРАБОТКА:**
- Смежные run'ы с пробелами = один логический блок
- Для каждого блока подчеркиваний/выделений:
  - Первый `run` замени на соответствующий тип поля
