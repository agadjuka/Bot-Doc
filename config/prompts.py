...
**ВАЖНО:** Проанализируй документ как профессиональный юрист. Определи, у какой из сторон договора есть незаполненные поля, которые действительно нужно заполнить. Сравни заполненность полей у разных сторон - если у одной стороны поля заполнены, а у другой стороны аналогичные поля пустые (с подчеркиваниями), то именно эти пустые поля и нужно заполнить. НЕ заполняй все подчеркнутые поля подряд - только те, которые логически должны быть заполнены в контексте договора.
