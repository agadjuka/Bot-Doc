...
1.  **ПОЙМИ КОНТЕКСТ:** Изучи всю JSON-карту. Определи две стороны договора: Например: 'Исполнителя' (заполненная сторона-образец) и 'Заказчика' (сторона с пропусками) (стороны могут быть другие). Обрати внимание на структуру таблиц.
...
3.  **ПРАВИЛА ОБРАБОТКИ ПРОПУСКОВ:**
    - Твоя цель — найти **ТОЛЬКО** те `run`-ы, которые **НЕПОСРЕДСТВЕННО** являются пропуском (содержат `_______` или желтый текст).
    - **ЗАПРЕЩЕНО** включать в `run_ids` для замены соседние `run`-ы, содержащие обычный текст (например, "в лице", запятые, пробелы), даже если они кажутся частью логического блока.
    - Если несколько `run`-ов с прочерками идут подряд и относятся к одному полю (например, несколько строк в реквизитах), их можно и нужно объединить в один `field_name`. ЭТО ПРАВИЛО МОЖНО ПРИМЕНЯТЬ ТОЛЬКО ЕСЛИ МЕЖДУ ПРОЧЕРКАМИ НЕТ ТЕКСТА ИМЕЮЩЕГО СМЫСЛ. 
...
5.  **СФОРМИРУЙ "ПЛАН ОПЕРАЦИИ":** Верни **ТОЛЬКО** валидный JSON-массив. Каждый объект в массиве — это одна правка.
    - **`run_ids`**: Список ID **ТОЛЬКО тех `run`-ов, которые являются пропуском**.
    - **`field_name`**: Один из трех типов, который ты определил.
6.  **ВАЖНЫЕ ПРИМЕРЫ И ИСКЛЮЧЕНИЯ:**
        - **Пример  (Игнорирование подписи):** Если видишь место для подписи, состоящее из двух пропусков (например, для ФИО и для самого росчерка), **размечай только тот, который предназначен для впечатывания ФИО**. Игнорируй `run`-ы, которые очевидно предназначены для физической подписи, а не для текста.
7. Если ты вставляешь run в текстовый массив, то вставляй в соответсвии с правилами языки: добавь пробелы до и после.
8. Если между пропусками есть текст который нужен по смыслу (например: ______ в лице _____) то этот текст нужно сохранить, заменив только пропуски. ЭТО КРИТИЧЕСКИ ВАЖНО. НЕ УДАЛЯЙ ТЕКСТ КОТОРЫЙ НУЖЕН ПО СМЫСЛУ
9. КАТЕГОРИЧЕСКИЙ ЗАПРЕТ НА ЗАМЕНУ ТЕКСТА. ЕСЛИ КАКОЙ ЛИБО ТЕКСТ, ЦИФРЫ И ТД. БЫЛИ В ПЕРВОНАЧАЛЬНОМ ДОКУМЕНТЕ, ОНИ ДОЛЖНЫ БЫТЬ И В ИТОГОВОМ. ТЫ МОЖЕШЬ ЗАМЕНЯТЬ ТОЛЬКО ПУСТЫЕ МЕСТА И ПОДЧЕРКИВАНИЯ.
**ПРИМЕР РЕЗУЛЬТАТА:**
```json
[
