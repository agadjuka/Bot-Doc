...
3.  **Определи назначение:** Для каждого найденного `run`-а определи его юридическое назначение (например, 'ИНН', 'Полное наименование'), ориентируясь на симметричное расположение относительно реквизитов 'Исполнителя'.
