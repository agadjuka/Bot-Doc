...
"""
    ...
    def _get_document_analysis_prompt(self, document_map: str) -> str:
        return f"""Ты — AI-юрист, эксперт по анализу юридических документов. Тебе предоставлена детальная карта документа в формате Markdown, где таблицы четко структурированы, а каждый текстовый фрагмент (`run`) имеет уникальный ID `[run-ID]`.
...
1.  **Проанализируй карту, уделяя особое внимание таблицам.** Изучи структуру. Определи 'Исполнителя' (заполненная колонка, твой образец) и 'Заказчика' (колонка с пропусками).
2.  **Найди все `run`-ы, которые являются пропусками (`_______` или желтый текст) и относятся только к 'Заказчику'.**
3.  **Для каждого пустого `run`-а определи, что это поле для реквизитов** - не нужно различать конкретные типы полей.
4.  **Сформируй "план правок":** Верни **ТОЛЬКО** валидный JSON-массив. Каждый объект в массиве должен представлять **ОДИН** `run`, который нужно изменить, и содержать два ключа:
    - `run_id`: Точный ID фрагмента (например, "run-282").
    - `field_name`: Всегда используй "реквизиты" для всех полей.

**ПРИМЕР РЕЗУЛЬТАТА:**
```json
[
  {{ "run_id": "run-51", "field_name": "реквизиты" }},
  {{ "run_id": "run-282", "field_name": "реквизиты" }},
  {{ "run_id": "run-283", "field_name": "реквизиты" }},
  {{ "run_id": "run-302", "field_name": "реквизиты" }}
]
```"""
